...
// // // src/pages/News/index.js - Updated with consistent loading
// // import React, { useState, useMemo } from 'react';
// // import Head from 'next/head';
// // import { motion } from 'framer-motion';
// // import Layout from '@/components/Layout';
...
// //   // Get unique categories
// //   const categories = useMemo(() => {
// //     const cats = ['All', ...new Set(newsData.map(item => item.categories).filter(Boolean))];
// //     return cats;
// //   }, [newsData]);
...
// //         (item.slug?.current?.toLowerCase().includes(search)) ||
// //         (item._rawBody?.toLowerCase().includes(search))
// //       );
// //     }
...
// //     <Layout>
// //       <Head>
// //         <title>News - Advanced Academic Center</title>
// //         <meta name="description" content="Latest news and updates from Advanced Academic Center" />
// //       </Head>

// //       <PageHero 
// //         title="Latest News"
// //         subtitle="Stay updated with the latest happenings, achievements, and announcements from Advanced Academic Center"
// //         tag="News & Updates"
// //       />
...
// // src/pages/News/index.js - Updated to use database
...
//     let processed = newsData.filter(news => {
//       // Basic filtering - ensure news has required fields
...
//         (news.slug?.current && news.slug.current.toLowerCase().includes(searchTerm.toLowerCase())) ||
//         (news._rawBody && news._rawBody.toLowerCase().includes(searchTerm.toLowerCase()));
...
//   }, [newsData, searchTerm, activeCategory, sortOrder]);

//   // Get unique categories for filter
//   const categories = ['All'];
//   if (newsData && Array.isArray(newsData)) {
//     const uniqueCategories = [...new Set(newsData.map(news => news.categories).filter(Boolean))];
...
//         <PageHero 
//           title="Latest News" 
//           subtitle="Stay updated with the latest happenings at Advanced Academic Center"
//           tag="News & Updates"
//         />
//         <ContentLoading text="Loading news..." />
//       </Layout>
//     );
//   }
...
//           <div className="text-center">
//             <p className="text-red-400 text-lg">Error loading news: {error}</p>
//             <button 
//               onClick={() => window.location.reload()} 
...
//         >
//           <p className="text-gray-400">
//             {filteredNews.length === 0 && newsData?.length > 0 
//               ? 'No news articles match your search criteria'
//               : `Showing ${filteredNews.length} news ${filteredNews.length === 1 ? 'article' : 'articles'}`
//             }
//           </p>
//         </motion.div>
...
//                 {newsData?.length === 0 ? 'No News Available' : 'No Results Found'}
//               </h3>
...
// src/pages/News/index.js - Combined Modern Premium UI with Database Functionality
import React, { useState, useMemo } from 'react';
import Head from 'next/head';
import Link from 'next/link';
import Image from 'next/image';
...
const NewsPage = () => {
  ...
  const normalizedNewsData = useMemo(() => {
    ...
    return newsData.map((news, index) => ({
      ...
      title: news.title || 'Untitled News',
      slug: news.slug || { current: news.description || '' },
      publishedAt: news.publishedAt || news.date || news.createdAt || news._createdAt || new Date().toISOString(),
      categories: news.categories || news.category || 'news',
      mainImage: news.mainImage || { asset: { url: news.image || null } },
      body: news.body || [],
      _rawBody: news._rawBody || news.content || '',
      ...
      <Layout>
        ...
        <div className="container mx-auto px-4 pb-24">
          <div className="text-center py-20">
            ...
            <h3 className="text-3xl font-bold text-white mb-4">Unable to Load News</h3>
            <p className="text-gray-400 text-lg mb-8">There was an error loading the news articles: {error}</p>
            <button
              onClick={() => window.location.reload()}
              className="group/btn px-6 py-3 bg-gradient-to-r from-emerald-500 to-emerald-600 text-white rounded-xl font-medium hover:shadow-lg transition-all duration-300 relative overflow-hidden hover:scale-105"
            >
