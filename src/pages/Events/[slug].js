// // src/pages/Events/[slug].js - Dynamic Event Detail Page with Modern Premium UI
...
//         <meta property="og:title" content={`${event.event} | AAC Events`} />
//         <meta property="og:description" content={event.description} />
//         <meta property="og:image" content={event.img || event.images?.[0]} />
//       </Head>
...
//           images={event.images || (event.img ? [event.img] : [])}
//           date={event.date}
...
// src/pages/Events/[slug].js - Updated to use database
import { useRouter } from 'next/router';
import Layout from '@/components/Layout';
import EventDetail from '@/components/Events/EventDetail';
...
export default function EventPage() {
  ...
  const event = eventsData?.find(e => {
    ...
    if (e.event) {
      const eventSlug = e.event.toLowerCase().replace(/[^a-z0-9]/g, '');
  ...
  if (loading) {
    return (
      <Layout>
        <ContentLoading text="Loading event details..." />
      </Layout>
    );
  }
