...
const EventCard = ({ event, isActive, onClick }) => {
  return (
    <motion.div 
      layout
      className={`relative cursor-pointer rounded-xl overflow-hidden shadow-lg ${isActive ? 'col-span-2 row-span-2' : 'col-span-1 row-span-1'}`}
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    ...
    >
      ...
      <div className="relative h-full p-6 flex flex-col justify-end z-10">
        <div className="mb-2">
          <span className="inline-block px-3 py-1 text-xs font-medium bg-white/20 backdrop-blur-sm text-white rounded-full">
            {event.date}
          </span>
        </div>
        ...
        <AnimatePresence>
          {isActive && (
            ...
            >
              ...
              <Link 
                href={event.path}
                className="inline-block px-4 py-2 bg-white/10 backdrop-blur-sm border border-white/20 text-white rounded-full hover:bg-white/20 transition-colors"
              >
                View Event
              </Link>
            </motion.div>
        ...
        <div className="container mx-auto max-w-6xl">
          ...
          <div className="mb-12">
            <div className="relative flex overflow-x-auto pb-4">
              <div className="absolute h-0.5 bg-gray-200 bottom-4 left-0 right-0"></div>
              {eventsData.map((event, index) => (
                ...
                >
                  ...
                  <div className={`w-4 h-4 rounded-full mt-1 mb-1 ${activeEventId === event.id ? 'bg-blue-600 scale-125' : 'bg-gray-300'} transition-all duration-300`}></div>
                  <span className={`text-sm font-medium ${activeEventId === event.id ? 'text-blue-600' : 'text-gray-700'} transition-colors duration-300`}>
                    {event.event}
                  </span>
                </motion.div>
              ))}
          ...
          >
            <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-2xl shadow-xl p-8 text-white">
              ...
              <p className="mb-6">Join our community of innovators and be the first to know about our latest activities.</p>
              <button className="px-6 py-3 bg-white text-blue-600 rounded-full font-medium hover:bg-gray-100 transition-colors">
